...
[comment]: # "  Copyright (c) 2016-2026 Splunk Inc."
[comment]: # ""
...
[comment]: # ""
The auth_token config parameter is for use with Splunk SOAR instances. If both the token and
username/password are given, the username and password will be used to authenticate to the Splunk 
SOAR instance.  
  
Note that the IP (or name) being used must match the allowed IP in the remote Splunk SOAR
instance's REST asset configuration.  
  
In case the **phantom_server** configuration parameter is set to the current Splunk SOAR instance,
 i.e., the Splunk SOAR server through which the app is being used, then the **verify_certificate** should be
...
the Splunk SOAR REST Overview documentation.  
  
If the value provided in the **phantom_server** configuration parameter is 0.0.0.0 then the **test
connectivity** passes successfully and the actions will run on the current Splunk SOAR instance, i.e.,
the server through which the app is being used.  
  
See [Add, remove, or replace certificates from the Splunk SOAR (On-premises) certificate store] (https://docs.splunk.com/Documentation/SOARonprem/6.4.0/Admin/AddOrRemoveCertificates) on how to create and verify a valid HTTPS certificate for your Splunk SOAR instance.  
  
For security reasons, accessing 127.0.0.1 is not allowed.  
  
For Unprivileged instances, the Device IP/Hostname configuration parameter needs to specify the port number
